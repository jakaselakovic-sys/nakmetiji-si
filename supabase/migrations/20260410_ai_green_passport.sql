...
CREATE INDEX IF NOT EXISTS idx_kmetije_fts_sl
  ON kmetije USING gin (
    to_tsvector('simple', unaccent(coalesce(ime, '') || ' ' || coalesce(opis, '') || ' ' || coalesce(kratki_opis, '')))
  );
...
CREATE TYPE IF NOT EXISTS znacka_tip AS ENUM (
  'regija',       -- earned by getting a region stamp
  'dosezek',      -- milestone (5 visits, 10 farms etc.)
  'sezonska',     -- time-limited seasonal badge
  'posebna'       -- manually awarded by admin
);
...
CREATE TYPE IF NOT EXISTS jezik_enum AS ENUM ('sl', 'en', 'de', 'it');
...
CREATE INDEX IF NOT EXISTS idx_prevodi_fts
  ON prevodi USING gin (
    to_tsvector('simple', unaccent(vsebina))
  );
...
CREATE TYPE IF NOT EXISTS transakcija_tip AS ENUM (
  'komisija',          -- 12% platform fee (auto-generated by trigger)
  'izplacilo',         -- payout to vendor via Stripe Connect
  'ai_agencija',       -- AI Agency service fee (billed monthly)
  'vracilo',           -- refund to guest
  'premium_narocnina'  -- vendor premium listing subscription
);

CREATE TYPE IF NOT EXISTS transakcija_status AS ENUM (
  'cakanje',    -- created, not yet processed
  'obdelana',   -- payment confirmed / payout sent
  'neuspesna',  -- payment/payout failed
  'vrnjena'     -- refunded
);
...
COMMENT ON COLUMN transakcije.ai_agencija IS
